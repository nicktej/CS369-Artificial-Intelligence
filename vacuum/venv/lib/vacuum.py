...
def init_state_agent():
    global direction
    global N, S, E, W
    return N, S, E, W, direction


def state_agent(percept, N, S, E, W, direction):
    if percept == 'dirty':
        return 'suck', N, S, E, W, direction
    else:
        if direction == 'west' and W < 2500:
            move = random.choice(['north', 'south', 'west'])
            W += 1
        elif direction == 'north' and N < 2500:
            move = random.choice(['north', 'east', 'west'])
            N += 1
        elif direction == 'south' and S < 2500:
            move = random.choice(['south', 'east', 'west'])
            S += 1
        elif direction == 'east' and E < 2500:
            move = random.choice(['north', 'south', 'east'])
            E += 1
        else:
            move = random.choice(['north', 'south', 'east', 'west'])
        return move, N, S, E, W, move
